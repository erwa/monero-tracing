...
// java ParseAnonSetDataWith0 TxInputsAll InputsReduced0.3
public class ParseAnonSetDataWith0 {
  ...
  private static void printReducedAnonSet(Map<Pair, Integer> origSzMap,
    ...
    // [0, 10]       [0, 11]
    //                0 because breach simulation may lead to contradictions
    ...
    for (Map.Entry<Integer,Map<Integer,Integer>> entry : anonSzMap.entrySet()) {
      ...
      for (int i = 0; i <= 11; i++) {
        Map<Integer,Integer> szCountMap = entry.getValue();
        if (i == 0) {
          if (szCountMap.containsKey(i)) {
            System.out.print("," + szCountMap.get(i));
          } else {
            System.out.print(",0");
          }
        } else if (entry.getValue().containsKey(i)) {
