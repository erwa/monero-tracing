...
// java TraceTxsReduceAnonSet TxInputsAll
public class TraceTxsReduceAnonSet {
  ...
  private static void printReducedAnonSet() {
    ...
    for (Map.Entry<Pair,Integer> traced : tracedInputs.entrySet()) {
      inputAnonSet.put(traced.getKey(), List.of(traced.getValue()));
    }

    // print out reduced anon set
    for (Map.Entry<Pair,List<Integer>> entry : inputAnonSet.entrySet()) {
      System.out.print(entry.getKey().tx + " "
        + entry.getKey().idx + " ");
      System.out.println(String.join(" ", // convert List<Integer> to List<String>
